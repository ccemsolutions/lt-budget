...
export default function SubcontractorsForm() {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="font-semibold text-gray-800">Subempreiteiros</h3>
          <p className="text-xs text-gray-500 mt-0.5">
            Preços unitários dos serviços subempreitados (S1–S18). Usados no cálculo das CPUs correspondentes.
          </p>
        </div>
        <button
          type="button"
      ...
      {fields.length === 0 ? (
        <div className="text-center py-10 text-gray-400 text-sm border-2 border-dashed rounded-xl">
          Nenhum subempreiteiro cadastrado. Clique em "+ Adicionar item" para começar.
        </div>
      ) : (
          ...
          <table className="text-sm w-full">
            <thead className="bg-gray-50 border-b">
              <tr>
                ...
                <th className="px-3 py-2 text-left text-xs font-semibold text-green-700 bg-green-50 min-w-[160px]">Observações</th>
              ...
              {fields.map((field, idx) => (
                <tr key={field.id} className="border-b last:border-b-0 hover:bg-gray-50">
                  <td className="px-2 py-1">
                    <input
                      {...register(`subcontractors.items.${idx}.code`)}
                      placeholder="S1"
                  ...
                  <td className="px-2 py-1">
                    <input
                      {...register(`subcontractors.items.${idx}.description`)}
                      placeholder="Ex.: Montagem de Torres"
                      className="w-full border rounded px-2 py-1 text-xs focus:outline-none focus:ring-1 focus:ring-blue-400"
                    />
                  </td>
                  <td className="px-2 py-1">
                    <input
                      {...register(`subcontractors.items.${idx}.unit`)}
                      placeholder="un"
                      className="w-full border rounded px-2 py-1 text-xs focus:outline-none focus:ring-1 focus:ring-blue-400"
                    />
                  </td>
                  <td className="px-2 py-1">
                    <input
                      type="number"
                      step="0.01"
                      {...register(`subcontractors.items.${idx}.unit_price`, { valueAsNumber: true })}
                  ...
                  <td className="px-2 py-1 bg-green-50">
                    <input
                      {...register(`subcontractors.items.${idx}.observations`)}
                      placeholder="Obs..."
                      className="w-full border rounded px-2 py-1 text-xs bg-green-50 focus:outline-none focus:ring-1 focus:ring-green-400"
                    />
                  </td>
