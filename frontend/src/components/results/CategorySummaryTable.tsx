...
export default function CategorySummaryTable({ summaries }: Props) {
  const directRows = summaries.filter(r => r.category !== 'Custos Indiretos')
  const indirectRow = summaries.find(r => r.category === 'Custos Indiretos')
  const hasIndirect = !!indirectRow

  const directTotal = directRows.reduce((s, r) => s + r.total_cost, 0)
  const grandTotal = summaries.reduce((s, r) => s + r.total_cost, 0)

  const Row = ({ row, i, shade }: { row: BudgetSummaryRead; i: number; shade?: string }) => (
    <tr className={shade ?? (i % 2 === 0 ? 'bg-white' : 'bg-gray-50')}>
  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full text-sm border-collapse">
        <thead>
          <tr className="bg-gray-100 text-gray-600 text-left">
            ...
            <th className="px-4 py-3 font-medium text-right">MAT / Outros</th>
        ...
        <tbody>
          ...
          {directRows.map((row, i) => (
            <Row key={row.category} row={row} i={i} />
          ))}

          {/* Subtotal direto (somente se tiver indireto) */}
          {hasIndirect && (
            <tr className="bg-blue-50 border-t border-blue-200 font-semibold text-sm">
              <td className="px-4 py-2 text-blue-800">Subtotal Direto</td>
              <td className="px-4 py-2 text-right">{fmt(directRows.reduce((s, r) => s + r.mo_cost, 0))}</td>
              <td className="px-4 py-2 text-right">{fmt(directRows.reduce((s, r) => s + r.vem_cost, 0))}</td>
              <td className="px-4 py-2 text-right">{fmt(directRows.reduce((s, r) => s + r.mat_cost, 0))}</td>
              <td className="px-4 py-2 text-right">{fmt(directRows.reduce((s, r) => s + r.sub_cost, 0))}</td>
              <td className="px-4 py-2 text-right">{fmt(directRows.reduce((s, r) => s + r.fd_cost, 0))}</td>
              <td className="px-4 py-2 text-right text-blue-800">{fmt(directTotal)}</td>
              <td className="px-4 py-2 text-right">{fmtHH(directRows.reduce((s, r) => s + r.manhours, 0))}</td>
              <td className="px-4 py-2 text-right">{grandTotal > 0 ? ((directTotal / grandTotal) * 100).toFixed(1) : 0}%</td>
            </tr>
          )}

          {/* Custos Indiretos */}
          {indirectRow && (
            <Row row={indirectRow} i={0} shade="bg-orange-50" />
          )}
        </tbody>
        ...
        <tfoot>
          <tr className="bg-blue-50 font-bold border-t-2 border-blue-200">
            ...
            <td className="px-4 py-3 text-right text-blue-700">{fmt(grandTotal)}</td>
      ...
      {/* Legenda para indiretos */}
      {hasIndirect && (
        <p className="text-xs text-gray-400 mt-2 italic">
          * Custos Indiretos — coluna MAT/Outros agrupa: canteiro, repúblicas, viagens, QSMS e mob/demob
        </p>
      )}
    </div>
  )
